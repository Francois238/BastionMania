...
// "ip/mask"
static REGEX_IPV4_CIDR: &str = r"^((25[0-5]|(2[0-4]|1\d|[1-9]|)\d)\.?\b){4}(/([0-9]|[1-2][0-9]|3[0-2]))$";
...
impl BastionConfig {
    pub fn new() -> BastionConfig {
        let bastion_private_key = env::var("BASTION_PRIVATE_KEY").expect("BASTION_PRIVATE_KEY must be set");
        ...
        let endpoint_match = re_ipv4_port.captures(&agent_endpoint)
            .expect(format!("Invalid endpoint : {}", agent_endpoint).as_str());
        let port = endpoint_match.get(4).unwrap().as_str();
        let port: u16 = port.parse().expect(format!("Invalid endpoint port : {}", port).as_str());
        if port < 1024 {
            panic!("Invalid endpoint port : {}", port);
        }

        let net_id: u8 = net_id.parse().expect(format!("Invalid net id : {}", net_id).as_str());
