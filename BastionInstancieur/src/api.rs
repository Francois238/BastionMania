...
async fn create_bastion(
    bastion_config: web::Json<BastionConfig>,
    instancieur_config: web::Data<InstancieurConfig>,
) -> impl Responder {
    ...
    match bastion_spec.create(instancieur_config.client.clone()).await {
        ...
        Err(e) => {
            log::error!(
                "Error creating Bastion {}: {}",
                bastion_config.bastion_id,
                e
            );
...
async fn delete_bastion(
    bastion_id: web::Path<String>,
    instancieur_config: web::Data<InstancieurConfig>,
) -> impl Responder {
    let bastion_id = bastion_id.into_inner();
    log::info!("Deleting bastion {}", bastion_id);
    match BastionSpec::delete(
        &format!("bastion-{bastion_id}"),
        instancieur_config.client.clone(),
    )
    .await
    {
        ...
        Err(e) => {
            ...
            return HttpResponse::InternalServerError()
                .body(format!("Error deleting Bastion {}", bastion_id));
        }
    }
...
pub fn config(cfg: &mut web::ServiceConfig) {
    cfg.service(create_bastion).service(delete_bastion);
}
