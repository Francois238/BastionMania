...
static DEFAULT_RESSOURCE_NET_ID: i32 = 0;
...
pub fn generate_ressource_freenetid(net_ids: &Vec<i32>) -> i32{
    let mut net_id = env::var("FIRST_RESSOURCE_NET_ID")
        .unwrap_or(DEFAULT_RESSOURCE_NET_ID.to_string())
        .parse::<i32>()
        .unwrap_or(DEFAULT_RESSOURCE_NET_ID);
    while net_ids.contains(&net_id){
        net_id += 1;
    }
    net_id
