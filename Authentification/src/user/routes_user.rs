...
#[post("/login")]
pub async fn sign_in_basic( credentials: web::Json<UserAuthentication>) -> Result<HttpResponse, ApiError> {
    ...
    if user.password.is_none() {  // l admin utilise keycloack
    ...
    let is_valid = verify_password(&user.password.as_ref().unwrap(), credentials.password.as_bytes())?;
...
#[patch("/users/{id}")]
async fn patch_user( id: web::Path<Uuid>, cred: web::Json<UserChangeCred>, ) -> Result<HttpResponse, ApiError> { //Un user peut modifier ses creds
...
#[post("/users/{id}/otp")]
async fn create_otp_user( id: web::Path<Uuid>, cred: web::Json<UserChangeCred> ) -> Result<HttpResponse, ApiError> { //Un user peut ajouter la 2FA a son compte
...
#[delete("/users/{id}")]
async fn delete_user( id: web::Path<Uuid>, cred: web::Json<UserSupprimer>, ) -> Result<HttpResponse, ApiError> { //Un admin peut modifier ses creds
