...
impl Keycloak {
    ...
    pub fn get_token(req: HttpRequest) -> Result<String, ApiError> {
        ...
        let header =general_purpose::STANDARD.decode(headerhttp).map_err(|_| ApiError::new(403,"Unauthorized".to_string()))?;

        let header = String::from_utf8(header).map_err(|_| ApiError::new(403,"Unauthorized".to_string()))?;
